...
for text in ['*.xyls', '*.axy', '*.corr', '*.match', '*.new', '*.wcs', '*.solved', '*.rdls', '*.png', '*.ps', '*.coo', '*.list', "*_resamp.fits", "*_resamp.weight.fits", "log*", "list_*", '*.ldac', '*.txt', '*.head', '*.ldac', 'wcs_*']:
...
def run_astrometry(filelist):
	...
	if os.path.exists('list_astrometry'):
		os.remove('list_astrometry')
		
	file_list='list_astrometry'
	list_files=group_similar_files(file_list, common_text='*.fits', exceptions='wcs_reference_')
...
+" --radius 1.0"+","+" --crpix-center"+" "+" --tweak-order 0"+","+" --overwrite"+" "+"--resort" \
...
def make_gaia_catalog(ra, dec, radius_deg, catalog_min_mag, catalog_max_mag, catname):
	job = Gaia.launch_job_async("SELECT * FROM gaiadr2.gaia_source AS g, gaiadr2.panstarrs1_best_neighbour AS pbest, gaiadr2.panstarrs1_original_valid AS ps1 WHERE g.source_id = pbest.source_id AND pbest.original_ext_source_id = ps1.obj_id AND CONTAINS(POINT('ICRS', g.ra, g.dec), CIRCLE('ICRS', %.4f, %.4f, %.4f))=1 AND ps1.r_mean_psf_mag > %.2f AND ps1.r_mean_psf_mag < %.2f AND pmra IS NOT NULL AND pmdec IS NOT NULL AND abs(pmdec) > 0 AND abs(pmdec) < 40 AND abs(pmra)>0 AND abs(pmra) < 40 AND ps1.n_detections > 6 AND pbest.number_of_mates=0 AND pbest.number_of_neighbours=1;"%(ra, dec, radius_deg, catalog_min_mag, catalog_max_mag), dump_to_file = False)
...
def run_sextractor(ctext):
    ...
    for file_name in list_files:
        command="sextractor %s -c %s -CATALOG_NAME %s -CATALOG_TYPE FITS_LDAC -PARAMETERS_NAME %s -MAG_ZEROPOINT 25.0" % (file_name, config_sex, file_name+'.ldac', param_sex)
        os.system(command)
...
def run_swarp(pxscale, ra, dec, gain, ctext):
    
    if os.path.exists('wcsfiles'):
        os.remove('wcsfiles')

    wcs_files='wcsfiles'
    list_wcsfiles=group_similar_files(wcs_files, common_text=ctext, exceptions='wcs_reference_')  
    cwd=os.getcwd()
    configfile=cwd+'/config.swarp'
    
    
    swarp_command="swarp -c %s -RESAMPLE Y -SUBTRACT_BACK Y -COMBINE N -PIXELSCALE_TYPE MANUAL -PIXEL_SCALE %.3f -CENTER %.4f,%.4f @%s -GAIN_DEFAULT %f" %(configfile, pxscale, ra, dec, wcs_files, gain)
    print("----------SwArP is running----------")
    print(swarp_command)
    os.system(swarp_command)
    
    
def run_swarp_ps1(pxscale, ra, dec, gain, ctext):
	...
	swarp_command="swarp -c %s -RESAMPLE Y -SUBTRACT_BACK N -COMBINE N -PIXELSCALE_TYPE MANUAL -PIXEL_SCALE %.3f -CENTER %.4f,%.4f @%s -GAIN_DEFAULT %.2f" %(configfile, pxscale, ra, dec, reference_files, gain)
	print("----------SwArP is running----------")
	print(swarp_command)
	os.system(swarp_command)
...
run_swarp(pxscale, ra, dec, gain, ctext='wcs_*.fits')
run_swarp_ps1(pxscale, ra, dec, gain_ps1, ctext='wcs_reference_*.fits')
copy_reference(ctext='wcs_reference_*.fits')
...
for file_name in group_similar_files('', common_text='*_resamp.fits'):
