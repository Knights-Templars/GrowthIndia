...
sys.path.append('/home/anirban.dutta/IMAGE_SUB/kde/growth_india_subtractions')
import ldactools as aw
import py_zogy
warnings.simplefilter('ignore')
astrom_scamp = '/home/anirban.dutta/IMAGE_SUB/kde/growth_india_subtractions/scamp.conf'
astrom_sex = '/home/anirban.dutta/IMAGE_SUB/kde/growth_india_subtractions/astrom.sex'
astrom_ref_sex='/home/anirban.dutta/IMAGE_SUB/kde/growth_india_subtractions/astrom_ref.sex'
astrom_swarp = '/home/anirban.dutta/IMAGE_SUB/kde/growth_india_subtractions/config.swarp'
psf_config_name = '/home/anirban.dutta/IMAGE_SUB/kde/growth_india_subtractions/photom.psfex'
sexpsfparam = '/home/anirban.dutta/IMAGE_SUB/kde/growth_india_subtractions/sexpsf.param'
...
def make_gaia_catalog(ra, dec, catalog_box_size, catalog_min_mag, catalog_max_mag, catname, writetext = False, writeldac = False):
	...
	if writetext:
		p.write(catname, format = 'ascii.no_header', include_names = ['ra', 'dec', 'g_mean_psf_mag','r_mean_psf_mag','i_mean_psf_mag','z_mean_psf_mag'])
	if writeldac:
...
def run_sextractor(sciname, refname):
	...
	sex_command = ['sex', '-c', astrom_ref_sex, refname, '-CATALOG_NAME', refname +'.cat', '-WEIGHT_TYPE', 'NONE', '-CHECKIMAGE_TYPE', 'NONE']
	try:
		rval = subprocess.call(sex_command)
	except subprocess.CalledProcessError as err:
...
def run_scamp(filelistname, ra ,dec, imagesize, sciname, refname):
	...
	make_gaia_catalog(ra, dec, imagesize, 10, 22, catname, writetext = False, writeldac = True)
...
def run_swarp(scifilename, reffilename, pxscale, imgpixsize, ra, dec):
	...
	swarp_command = "swarp %s -c %s -RESAMPLE Y -RESAMPLE_DIR . -SUBTRACT_BACK Y -WEIGHT_TYPE MAP_WEIGHT -WEIGHT_IMAGE %s -PIXELSCALE_TYPE MANUAL -PIXEL_SCALE %.3f -IMAGE_SIZE %d -CENTER %.8f,%.8f -COMBINE N -IMAGEOUT_NAME %s -WEIGHTOUT_NAME %s"%(reffilename, astrom_swarp, reffilename + '.weight', pxscale, imgpixsize, ra , dec, reffilename + '.resamp', reffilename + '.resamp.weight')
	
	print(swarp_command)
	os.system(swarp_command)
	
	sciresampname = scifilename + '.resamp'
	refresampname = reffilename + '.resamp'
...
def get_psf(filename):
	...
	sex_command = ['sex', '-c', astrom_sex, filename, '-CATALOG_NAME', filename+'.cat', '-WEIGHT_TYPE', 'NONE']
	try:
		rval = subprocess.call(sex_command)
	except subprocess.CalledProcessError as err:
...
def run_zogy(sciname, refname, scipsf, refpsf):
	...
	sci_gain = 1.6
	scihdu.close()
	sciweighthdu = fits.open(sciname + '.weight')
	...
	ref_gain = 1.04
	refhdu.close()
	refweighthdu = fits.open(refname + '.weight')
	...
	diff_hdu.writeto(sciname + '.diff.fits', overwrite = True)
	
	diff_psf_hdu = fits.PrimaryHDU(P_D)
	diff_psf_hdu.writeto(sciname + '.diff.psf.fits', overwrite = True)	
	...
	scorr_hdu.writeto(sciname + '.scorr.fits', overwrite = True)
